...
export function IncomeForm({ initialData, onSubmit, onCancel }: IncomeFormProps) {
  const [source, setSource] = useState(initialData?.source ?? "Gaji");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!source.trim()) return setError("Sumber pemasukan wajib diisi.");
    ...
    setIsLoading(true);
    try {
      await onSubmit({ source: source.trim(), amount: numericAmount, date, note: note.trim() || undefined });
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div>
        <label className="mb-1 block text-sm font-medium">Sumber Pemasukan</label>
        <input
          value={source}
